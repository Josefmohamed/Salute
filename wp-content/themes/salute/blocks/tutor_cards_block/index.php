...
if (isset($block)) {
  ...
  if (function_exists('is_admin') && is_admin()) {
    if (wp_is_json_request() || (defined('REST_REQUEST') && REST_REQUEST) || (function_exists('get_current_screen') && get_current_screen()->is_block_editor())) {
      echo '<img width="100%" height="100%" src="' . get_template_directory_uri() . '/blocks/tutor_cards_block/screenshot.png">';
      return;
    }
  }
}
  ...
  <div class="container">
    <?php if ($programmatic_or_manual === 'manual') {
      ...
      <div class="tutors-grid">
        <?php
        $cards = get_field("tutor_card");
        if (is_array($cards)) {
          foreach ($cards as $card) {
            get_template_part("partials/tutor-card", "", ["post_id" => $card->ID]);
          }
        }
        ?>
      </div>
    <?php } elseif (isset($the_query) && $the_query->have_posts()) { ?>
      <div class="tutors-grid">
        <?php while ($the_query->have_posts()) {
          $the_query->the_post();
          get_template_part("partials/tutor-card", "", ["post_id" => get_the_ID()]);
        } ?>
