...
<section id="<?= esc_attr($id) ?>" class="<?= esc_attr($className) ?>">
    <svg class="get-in-touch-svg animation-fade-me-up" width="1728" height="56" viewBox="0 0 1728 56" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <div class="content-wrapper">
        <div class="left-content">
            <div class="top-content">
                <?php if ($title) { ?>
                    <h4 class="salute-h4 title uppercase-text fw-800 animation-fade-me-up"><?= $title ?></h4>
                <?php } ?>
                <?php if ($description) { ?>
                    <div class="description salute-h6 animation-fade-me-up"><?= $description ?></div>
                <?php } ?>
            </div>
            <div class="contact-social-links">
                ...
                <div class="social-links-follow column">
                    <?php if ($follow_us) { ?>
                        <div class="paragraph-26 fw-700 follow-us animation-fade-me-up"><?= $follow_us ?></div>
                    <?php } ?>
                            ...
                            <?php while (have_rows('social_links')) {
                                ...
                                <a href="<?= $url ?>" target="_blank" class="social-link" aria-label=" (opens in a new tab)">
                                    <?php if (!empty($icon) && is_array($icon)) { ?>
                                        <picture class="icon-wrapper cover-image animation-fade-me-up">
        ...
        <?php if ($form) {?>
            <div class="contact-us-form-wrapper animation-fade-me-up contact-details">
                <div class="column form-info">
                    <?php if ($title_form) { ?>
                        <h6 class="salute-h6 fw-800 uppercase-text form-title animation-fade-me-up"><?= $title_form ?></h6>
                    <?php } ?>
                    <?php if ($description_form) { ?>
                        <div class="paragraph-22 animation-fade-me-up"><?= $description_form ?></div>
                    <?php } ?>
                </div>
